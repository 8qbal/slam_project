...
class SpotRewardsCfg:
    ...
    # 2. 把你寫好的「視覺牆壁懲罰」加進來，權重給重一點！
    visual_wall_penalty = RewardTermCfg(
        func=visual_wall_penalty, # 呼叫你上面寫好的 function
        weight=-0.1, # 扣分要夠重，蓋過追求速度的渴望
        params={
            "sensor_cfg": SceneEntityCfg("train_camera"),
            "safe_distance": 0.6, # 進入 0.6m 危險範圍就開始按平方扣分
        },
    )

    # 3. (選用) 加入開闊空間獎勵，鼓勵它往沒有牆壁的地方微調方向
    open_space_seeker = RewardTermCfg(
        func=open_space_seeker_reward,
        weight=0.5, # 給點甜頭，引導它轉向
        params={"sensor_cfg": SceneEntityCfg("train_camera")},
    )
    gait = RewardTermCfg(
...
class SpotRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    ...
    def __post_init__(self):
        ...
        # self.scene.robot.init_state.pos = (0.0, 0.0, 0.63)

        self.scene.robot.init_state.pos = (0.0, 2.0, 0.63)
        ...
        #     spawn=sim_utils.UsdFileCfg(
        #         usd_path="/home/bernie/Isaac_lab/project/spot_vslam/source/spot_vslam/spot_vslam/my_project/spot_new/Collected_spot/four_big_walls.usd"
        #     ),
        #     init_state=AssetBaseCfg.InitialStateCfg(pos=(0.0, 0.0, 0.0)),
        # )
...
class SpotHighLevelTrainEnvCfg(SpotRoughEnvCfg):
    ...
    def __post_init__(self):
        ...
        # warehouse
        # self.scene.robot.init_state.pos = (-4.0, 15.5, 0.6)  
