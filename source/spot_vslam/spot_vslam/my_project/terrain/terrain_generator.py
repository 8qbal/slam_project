...
from isaaclab.sim.sim_utils import spawn_usd
...
class TerrainGenerator:
    ...
    def _generate_random_terrains(self):
        ...
        for index in range(self.cfg.num_rows * self.cfg.num_cols):
            ...
            if getattr(sub_terrain_cfg, "walls_usd_path", None):
                walls_prim_path = f"/World/EnvWalls_{row}_{col}"
                spawn_usd(
                    stage=self.stage,  # 你需要傳 stage
                    usd_path=sub_terrain_cfg.walls_usd_path,
                    prim_path=walls_prim_path,
                    translation=origin
                )
    ...
    self, mesh: trimesh.Trimesh, origin: np.ndarray, row: int, col: int, sub_terrain_cfg: SubTerrainBaseCfg
        ...
        if getattr(sub_terrain_cfg, "walls_usd_path", None):
            walls_prim_path = f"/World/EnvWalls_{row}_{col}"
            spawn_usd(
                stage=self.stage,  # 你需要在 TerrainGenerator 初始化時傳 self.stage
                usd_path=sub_terrain_cfg.walls_usd_path,
                prim_path=walls_prim_path,
                translation=origin + transform[:3, -1]
            )
